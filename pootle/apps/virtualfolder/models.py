...
class VirtualFolder(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # For each store create all VFolderCachedTreeItem tree structure up to
        # its adjusted vfolder location.
        for store in vfolder_stores_set:
            VFolderCachedTreeItem.objects.get_or_create(
...
class VFolderCachedTreeItemManager(models.Manager):
    use_for_related_fields = True

    def get_queryset(self):
        return super(VFolderCachedTreeItemManager, self) \
            .get_queryset().select_related('vfolder')


class VFolderCachedTreeItem(models.Model, CachedTreeItem):
    ...
    objects = VFolderCachedTreeItemManager()
    ...
    def save(self, *args, **kwargs):
        ...
        if self.directory.pootle_path.count('/') > self.vfolder.location.count('/'):
            parent, created = VFolderCachedTreeItem.objects.get_or_create(
                directory=self.directory.parent,
                vfolder=self.vfolder,
            )
            self.parent = parent

        super(VFolderCachedTreeItem, self).save(*args, **kwargs)
    ...
    def delete(self, *args, **kwargs):
        ...
        for item in self.child_vf_treeitems.iterator():
            # Store children are deleted by the regular folders.
            item.delete()

        super(VFolderCachedTreeItem, self).delete(*args, **kwargs)
    ...
    def get_children(self):
        ...
        result.extend([item for item in self.child_vf_treeitems.iterator()])
        return result

    def get_stats(self, include_children=True):
        result = super(VFolderCachedTreeItem, self).get_stats(
    ...
    def all_pootle_paths(self):
        ...
        We only return the paths for the VFolderCachedTreeItem tree since we
        ...
        for location in vf.all_locations:
            ...
            for filename in vf.filter_rules.split(","):
                if pootle_path == "".join([location, filename]):
                    ...
                    # Create missing VFolderCachedTreeItem tree structure after
                    # adding this new unit.
                    vfs, created = VFolderCachedTreeItem.objects.get_or_create(
                        directory=instance.store.parent,
                        vfolder=vf,
                    )

                    if not created:
                        # The VFolderCachedTreeItem already existed, so
                        # calculate again the stats up to the root.
                        vfs.update_all_cache()
