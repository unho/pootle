...
def make_vfolder_item(vf_treeitem):
    return {
        'href_all': vf_treeitem.get_translate_url(),
        'href_todo': vf_treeitem.get_translate_url(state='incomplete'),
        'href_sugg': vf_treeitem.get_translate_url(state='suggestions'),
        'href_critical': vf_treeitem.get_critical_url(),
        'title': vf_treeitem.vfolder.name,
        'code': vf_treeitem.code,
        'priority': vf_treeitem.vfolder.priority,
        'is_grayed': not vf_treeitem.is_visible,
        'icon': 'folder',
    }
...
def get_vfolders(directory, all_vfolders=False):
    ...
    """
    return [make_vfolder_item(item)
            for item in directory.vf_treeitems.order_by('-vfolder__priority').iterator()
            if all_vfolders or item.is_visible]
